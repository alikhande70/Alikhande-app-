...
//--- Clock domains ------------------------------------------------------------
// TimeTradeServer() is the *broker's* wall clock, which for LiteFinance and most
// FX brokers is GMT+2/+3, not UTC. The desk stamps its ledger in real UTC, so
// any value that will be compared against desk timestamps -- send windows,
// history coverage, evidence times -- must be UTC or the comparison silently
// measures the broker's timezone offset instead of elapsed time.
//
// The sign of that error decides which way it fails: a server ahead of UTC makes
// absence impossible to conclude, and a server behind UTC makes false absence
// possible. Neither is acceptable, so UTC crosses the wire and the offset is
// reported alongside it for session reasoning that genuinely wants server time.
