...
import { evaluateDurableRegisteredResearch } from './evaluation-orchestration.js';
...
describe('durable registered research orchestration', () => {
  it('runs the public deterministic Brain evaluator only after Desk resolves ledger provenance', () => {
  ...
  it('fails before invoking the evaluator when the requested family is not durable', () => {
    const ledger = memoryLedger();
  ...
  it('preserves Brain anti-backdating failure when evidence predates durable registration', () => {
    const ledger = memoryLedger();
    const sealed = registration();
